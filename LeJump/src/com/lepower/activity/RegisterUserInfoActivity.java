...
public class RegisterUserInfoActivity extends BaseActivity {
	...
	@Event(value = { R.id.txt_address, R.id.txt_birthday, R.id.txt_height,
			R.id.txt_weight, R.id.btn_finish, R.id.img_userhead,
			R.id.login_reback_btn })
	private void click(View view) {
		...
		case R.id.btn_finish: {
			if (basicUserInfo()) {
				...
				String[] sp = addressTextView.getText().toString().trim()
						.split(",");
				province = sp[0];
				...
				HttpUtils.post(LeUrls.REGISITER_URL, params,
	...
	protected void heightDialog() { // 身高选择对话框
		if (alertDialogHeight == null) {
			ArrayAdapter<String> heightAdapter = new ArrayAdapter<String>(
					RegisterUserInfoActivity.this, R.layout.list_item_view,
	...
	public boolean basicUserInfo() {
		...
		if (birthdayTextView.getText().toString().trim().equals("")) {
	...
	private void initData() {
		...
		if (intent.getBooleanExtra("qq", false)) {
			...
			addressTextView.setText(qqInfo.getProvince() + ","
					+ qqInfo.getCity());
